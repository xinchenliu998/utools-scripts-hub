...
│   ├── logo.png             # 插件图标
│   └── preload/
│       └── services.js      # Node.js 服务（文件操作、脚本执行等）
├── src/
│   ├── App.vue              # 主应用组件
│   ├── main.ts              # 入口文件
│   ├── main.css             # 全局样式
...
1. 在 `src/` 目录下创建新组件
2. 在 `App.vue` 中添加路由
3. 在 `public/plugin.json` 中添加功能入口

### 扩展脚本打开方式

在 `public/preload/services.js` 中的 `openWithRule` 和 `openWithApp` 函数中可以扩展文件打开逻辑。
