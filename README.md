...
- 支持自定义应用启动参数
- 支持启用/禁用规则（禁用的规则不会参与匹配）
- 支持拖拽调整规则顺序（规则按顺序匹配，顺序很重要）
- 未匹配规则时使用系统默认应用打开

### 🎨 个性化设置

- **多语言支持**: 支持中文和英文界面切换
- **主题色自定义**: 可自定义主题颜色，适应不同偏好
- **按钮颜色配置**: 自定义主要、危险、警告、成功、默认按钮的颜色
- **文件夹排除**: 配置扫描时默认排除的文件夹（如 `node_modules`、`.git` 等）
...
**启用/禁用脚本**: 在脚本列表中，可以点击启用/禁用按钮来切换脚本的启用状态。禁用的脚本不会在运行界面显示，也不会被搜索到。

**调整脚本顺序**: 在脚本列表中，可以通过拖拽列表项来调整脚本的显示顺序。拖拽时鼠标会显示移动图标，搜索状态下拖拽功能会自动禁用。
...
**启用/禁用规则**: 在规则列表中，可以点击启用/禁用按钮来切换规则的启用状态。禁用的规则不会参与文件匹配。

**调整规则顺序**: 在规则列表中，可以通过拖拽列表项来调整规则的匹配顺序。规则按照列表顺序依次匹配，找到第一个匹配的规则即停止，因此顺序很重要。拖拽时鼠标会显示移动图标，搜索状态下拖拽功能会自动禁用。
...
1. 在设置界面点击「设置」按钮（或使用快捷键）
2. 设置对话框包含以下标签页：
   - **外观**: 选择语言（中文/英文）、设置主题色
   - **排除**: 配置扫描时默认排除的文件夹，支持自定义添加/删除
   - **按钮**: 自定义各种按钮的颜色（主要、危险、警告、成功、默认）
3. 点击「保存」应用设置，点击「重置」恢复默认值
...
- [重构文档](./docs/REFACTORING.md) - 重构历史和改进点
