...
│   │       ├── settings/           # 设置对话框子组件
│   │       │   ├── AppearanceTab.vue  # 外观设置（语言、主题色）
│   │       │   ├── ExcludeTab.vue     # 排除文件夹设置
│   │       │   └── ButtonsTab.vue     # 按钮颜色设置
│   │       └── common/             # 共享组件
...
`src/composables/useSettings.ts` 提供了以下函数和响应式数据：
...
- `settings`: 设置对象，包含:
  ...
  - `themeColor`: 主题色
  - `buttonColors`: 按钮颜色配置
  - `defaultExcludeFolders`: 默认排除的文件夹列表

#### 函数

- `loadSettings()`: 从配置中加载设置
- `saveSettings()`: 保存设置到配置
- `resetSettings()`: 重置为默认值
- `updateDefaultExcludeFolders(folders)`: 更新默认排除文件夹
- `updateThemeColor(color)`: 更新主题色
- `updateButtonColor(variant, color)`: 更新按钮颜色
- `getDefaultExcludeFolders()`: 获取默认排除文件夹
- `getButtonColors()`: 获取按钮颜色配置
- `setLocale(locale)`: 切换语言
- `localeMessages`: 当前语言消息对象
